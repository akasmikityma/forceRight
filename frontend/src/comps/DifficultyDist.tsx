...
// const COLORS = ["#17b978", "#ffb703", "#e63946"];
...
const DifficultyStats: React.FC = () => {
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md flex items-center gap-4 relative"> {/* Added relative for absolute positioning of Skeleton */}
      {isLoading ? ( // Conditionally render Skeleton
        <div className="grid grid-cols-[auto_1fr] gap-4 w-full"> {/* Grid Container */}
        <div className="relative h-24 w-24"> {/* PieChart Skeleton */}
          <Skeleton className="h-full w-full rounded-full bg-gray-200 absolute inset-0" />
        </div>
        <div className="flex flex-col gap-2"> {/* StatBoxes Skeletons */}
...
const StatBox: React.FC<{ label: string; value: number; color: string }> = ({
  ...
  <div className={`flex justify-between px-3 py-1 gap-1 bg-gray-100 rounded-lg ${color} text-gray-700`}> {/* Adjusted styling for StatBox */}
...
// i just want that circle of distribution 
// in the middle there is total count >> at the right side of that circle three divs are there for the counts of [easy , medium , hard];
